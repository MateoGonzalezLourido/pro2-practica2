...
tPosP next(tPosP p, tListP L);            /*devolver siguiente nodo ocupado;
        ...
        postcondicion: no hay
        */
tPosP previous(tPosP p, tListP L);        /* devolver anterior nodo ocupado;
       recibe de entradas el valor de la lista y el nodo al que buscaremos su siguiente
 ...
 */
tItemP getItem(tPosP pos, tListP L); /*recuperar info proyecto de un nodo de la
...
tListP updateItem(tItemP newData, tPosP pos, tListP *L); /*actualizar un nodo de
...
*/
tPosP findItem(tProjectName data, tListP L);   /*buscar indice proyecto;
recibe de entradas el nombre del proyecto y el valor de la lista
salida: la info del nodo / null
precondicion:ho hay
postcondicion: ho hay */
tListP deleteAtPosition(tPosP pos, tListP *L); /*borrar proyecto
